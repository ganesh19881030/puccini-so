...
type DbTopologyTemplate struct {
	DbToscaObject // embedded structure that has the basic functionality
}

func (ntemp *DbTopologyTemplate) Convert(responseData *interface{}, readerName string) interface{} {
	return AddReaderNameToData(responseData, readerName)
}

func (ntemp *DbTopologyTemplate) DbRead(dgt *dgraph.DgraphTemplate, fieldData *interface{}, uid string, key string) ard.Map {
	var compMap ard.Map
	var err error
	var name string
...
func (ntemp *DbTopologyTemplate) DbBuildInsertQuery(dataObject interface{}) (string, error) {
