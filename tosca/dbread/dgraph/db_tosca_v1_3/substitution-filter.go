...
type DbSubstitutionFilter struct {
	DbToscaObject // embedded structure that has the basic functionality
}

func (ntemp *DbSubstitutionFilter) Convert(responseData *interface{}, readerName string) interface{} {
	return AddReaderNameToData(responseData, readerName)
}

func (ntemp *DbSubstitutionFilter) DbRead(dgt *dgraph.DgraphTemplate, fieldData *interface{}, uid string, key string) ard.Map {
	var compMap ard.Map
	var err error
	var name string
...
func (ntemp *DbSubstitutionFilter) DbBuildInsertQuery(dataObject interface{}) (string, error) {
